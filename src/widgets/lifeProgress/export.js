...
export const generateLifeProgressHTML = (config) => {
  ...
  <style>
    * {
    ...
    .progress-fill {
      ...
      ${config.useGlowEffect ? `box-shadow: ${JAZER_BRAND.glow};` : ''}
    }
    ...
    @media (prefers-color-scheme: dark) {
      ${config.appearanceMode === 'system' ? `
        ...
        .progress-fill {
          background: ${config.useGradientBars ? JAZER_BRAND.gradient : config.barColorDark};
        }
      ` : ''}
    }
  </style>
...
export const generateLifeProgressScript = (config) => {
  return `
