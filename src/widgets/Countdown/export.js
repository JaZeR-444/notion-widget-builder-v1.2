...
export const generateCountdownHTML = (config) => {
  const isDark = config.appearance === 'dark';
  const colors = isDark ? config.darkMode : config.lightMode;
  const bgColor = config.useTransparentBg ? 'transparent' : (isDark ? JAZER_BRAND.colors.nightBlack : JAZER_BRAND.colors.stardustWhite);
  ...
  <style>
    * {
    ...
    #event-title {
      ...
      font-weight: bold;
      margin-bottom: 24px;
      ${config.textShadows ? `text-shadow: 0 0 ${JAZER_BRAND.glowBlur} rgba(0,0,0,0.1);` : ''}
    }
    ...
    .countdown-digit {
      ...
      position: relative;
      ${config.textShadows ? `text-shadow: 0 0 ${JAZER_BRAND.glowBlur} rgba(0,0,0,0.3);` : ''}
    }
...
export const generateCountdownScript = (config) => {
  const isDark = config.appearance === 'dark';
  const colors = isDark ? config.darkMode : config.lightMode;
  ...
  return `
    ...
    const JAZER_BRAND_COLORS = ${JSON.stringify(JAZER_BRAND.colors)};
