...
export const generateQuotesHTML = (config) => {
  ...
  const quoteCardStyle = `
    ...
    ${config.gradientQuoteCardBorder ? `
      ...
      background-image: ${JAZER_BRAND.gradient}, ${bgColor};
      background-origin: border-box;
      background-clip: padding-box, border-box;
    ` : ''}
    ${config.glowQuoteCard ? `box-shadow: ${JAZER_BRAND.glow};` : ''}
  `;
  ...
  <style>
    * {
    ...
    .refresh-button:hover {
      transform: scale(1.1);
      ${config.glowRefreshIconHover ? `filter: drop-shadow(${JAZER_BRAND.glow});` : ''}
    }
...
export const generateQuotesScript = (config) => {
  return `
    const exampleQuotes = [
