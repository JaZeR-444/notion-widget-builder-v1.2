...
export const generateImageGalleryHTML = (config) => {
  const images = config.images.filter(Boolean);
  if (images.length === 0) {
    return `<div style="display: flex; align-items: center; justify-content: center; height: 100%; width: 100%; color: ${JAZER_BRAND.colors.softSlate};">No images configured.</div>`;
  }
...
export const generateImageGalleryScript = (config) => {
